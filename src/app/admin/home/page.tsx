...
export default function AdminHome() {
  ...
  const [openSections, setOpenSections] = useState<Record<string, boolean>>({
    slideshow: false, // collapsed by default — content sections visible immediately
    hero:      false,
    press:     false,
    about:     true,  // open by default so user sees where to edit bio
    contact:   false,
  })
  function toggleSection(key: string) {
    setOpenSections(prev => ({ ...prev, [key]: !prev[key] }))
  }
  function AccordionHeader({ id, label, sub }: { id: string; label: string; sub?: string }) {
    ...
    return (
      ...
      >
        <span>
          <span style={{ fontFamily: 'Georgia, serif', fontSize: 'var(--fs-lg)' }}>{label}</span>
          {sub && <span style={{ fontSize: 'var(--fs-xs)', color: 'var(--color-muted)', marginLeft: '0.75rem' }}>{sub}</span>}
        </span>
        <span style={{ fontSize: 'var(--fs-sm)', color: 'var(--color-muted)' }}>{open ? '▲' : '▼'}</span>
      </button>
    )
  }
  ...
  return (
    <div style={{ padding: 'clamp(1.5rem, 3vw, 3rem)', width: '100%', boxSizing: 'border-box' }}>
      ...
      <div style={{ marginBottom: '2rem' }}>
        <p style={{ fontSize: 'var(--fs-xs)', color: 'var(--color-accent)', letterSpacing: '0.1em', textTransform: 'uppercase', marginBottom: '0.4rem' }}>Admin</p>
        <h1 style={{ fontFamily: 'Georgia, serif', fontWeight: 400, fontSize: 'clamp(var(--fs-xl), 4vw, var(--fs-3xl))', margin: 0 }}>Startsidan</h1>
        <p style={{ color: 'var(--color-muted)', fontSize: 'var(--fs-sm)', marginTop: '0.35rem' }}>
          {loading ? 'Laddar…' : 'Klicka på en sektion för att redigera.'}
        </p>
      </div>
      ...
      {message && (
        <div style={{
          ...
          padding: '0.75rem 1rem', fontSize: 'var(--fs-sm)', marginBottom: '1rem',
        }}>{message.text}</div>
      )}
      {contentMsg && (
        <div style={{
      ...
      {loading ? <p style={{ color: 'var(--color-muted)' }}>Laddar…</p> : (
        <>
          {/* ══ ACCORDION: HERO SLIDESHOW ══ */}
          <AccordionHeader
            id="slideshow"
            label="Hero Slideshow"
            sub={`${slides.length} bilder${isDirty ? ' · ● Osparade ändringar' : ''}`}
          />
          {openSections.slideshow && (
            <div style={{ border: '1px solid var(--color-border)', borderTop: 'none', padding: '1.5rem', marginBottom: 0 }}>
              {/* Controls */}
              ...
              {slides.length > 0 && (
                <p style={{ fontSize: 'var(--fs-xs)', color: 'var(--color-muted)', marginBottom: '1rem', lineHeight: 1.6 }}>
                  Tips: <strong style={{ color: 'var(--color-text)' }}>Klicka på motivet</strong> i en bild för att justera fokuspunkten. Klicka på ◎ för att återgå till mitten.
                </p>
              )}

              {/* Preview */}
              {previewing && previewOrder.length > 0 && (
                <div style={{ marginBottom: '1.5rem', border: '1px solid var(--color-border)', background: '#000', position: 'relative', borderRadius: 2 }}>
          ...
          {/* ══ ACCORDION: HERO TEXT ══ */}
          <AccordionHeader id="hero" label="Hero — namn & tagline" sub="Konstnärens namn och underrubrik i hero-bilden" />
          {openSections.hero && (
            <div style={{ border: '1px solid var(--color-border)', borderTop: 'none', padding: '1.5rem' }}>
          ...
          {/* ══ ACCORDION: PRESS QUOTE ══ */}
          <AccordionHeader id="press" label="Recension — citat & ljudklipp" sub="Blockquote, namn, källa och inbyggd audiospelare" />
          {openSections.press && (
            <div style={{ border: '1px solid var(--color-border)', borderTop: 'none', padding: '1.5rem' }}>
              <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(min(100%, 420px), 1fr))', gap: '1.5rem' }}>
          ...
          {/* ══ ACCORDION: ABOUT ARTIST (open by default) ══ */}
          <AccordionHeader id="about" label="Om konstnären — bio & statistik" sub="Biografitext och sifferrutor" />
          {openSections.about && (
            <div style={{ border: '1px solid var(--color-border)', borderTop: 'none', padding: '1.5rem' }}>
              <Field label="Biografitext" hint="Kortfattad presentation. Visas till vänster i Om-sektionen på startsidan.">
              ...
              <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(180px, 1fr))', gap: '1rem' }}>
                {([
                  ['statNActive', 'År aktiv (siffra)'],
                  ['statNPublic', 'Offentliga verk (siffra)'],
                  ['statNCountries', 'Länder (siffra)'],
                  ['statNBorn', 'Födelseår (siffra)'],
              ...
              <p style={{ fontSize: '0.68rem', color: 'var(--color-muted)', marginTop: '0.75rem', lineHeight: 1.5 }}>
                Etiketterna (t.ex. "År aktiv", "Länder") redigeras i lokaliseringsfiler.
              </p>
            </div>
          )}

          {/* ══ ACCORDION: CONTACT ══ */}
          <AccordionHeader id="contact" label="Kontakt — bakgrundsbild" sub="Bilden bakom kontakt-CTA-sektionen längst ner" />
          ...
          {/* Save content button — always visible */}
          <div style={{ borderTop: '2px solid var(--color-border)', paddingTop: '1.5rem', marginTop: '2rem', display: 'flex', gap: '1rem', alignItems: 'center' }}>
