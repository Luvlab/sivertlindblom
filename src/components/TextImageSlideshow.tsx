...
export default function TextImageSlideshow({ images, title, thumbnailAspect = '3/4' }: Props) {
  ...
  // Keyboard navigation for inline slideshow — disabled while lightbox is open
  useEffect(() => {
    if (lbOpen) return
  ...
  // LightboxImage format for the full-screen overlay
  const lbImages: LightboxImage[] = images.map((url, i) => ({
    url,
    alt: `${title} — sida ${i + 1}`,
  }))
  ...
  return (
    <>
      <div>
        ...
        <div
          style={{ position: 'relative', background: '#0a0a0a', border: '1px solid var(--color-border)', cursor: 'zoom-in' }}
          onClick={() => setLbOpen(true)}
          title="Klicka för helskärm"
        >
          ...
          <div style={{
            position: 'absolute',
            bottom: images.length > 1 ? '2rem' : '0.5rem',
            right: '0.6rem',
          ...
          {images.length > 1 && (
            <>
              <button onClick={e => { e.stopPropagation(); prev() }} aria-label="Föregående" style={{
              ...
              <button onClick={e => { e.stopPropagation(); next() }} aria-label="Nästa" style={{
